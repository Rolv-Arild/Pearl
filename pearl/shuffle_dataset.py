...
def main(args):
    ...
    for file in pbar:
        shard = EpisodeData.load(os.path.join(input_dir, file))
    ...
    while current_index < total_samples:
        ...
        for n, file in enumerate(pbar):
            old_shard = EpisodeData.load(os.path.join(input_dir, file))
    ...
    for n, file in enumerate(val_files):
        shutil.copy(os.path.join(input_dir, file), os.path.join(output_dir, f"validation_shard_{n}.npz"))


if __name__ == '__main__':
    while len(os.listdir(r"D:\rokutleg\ngp_dataset\RLCS_22_23")) < 49:
        time.sleep(60)
